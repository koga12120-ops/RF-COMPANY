...
button_html = """                      <button onClick={() => printReceipt(sale, String(sales.length - index).padStart(6, '0'))} className="p-1.5 bg-indigo-50 text-indigo-600 rounded hover:bg-indigo-100" title="چاپکردنی تەنها ئەمە"><Printer size={16} /></button>
...
content = re.sub(r'<button onClick=\{\(\) => printReceipt\(sale, String\(sales\.length - index\)\.padStart\(6, \'0\'\)\)\} className="p-1\.5 bg-indigo-50 text-indigo-600 rounded hover:bg-indigo-100"><Printer size=\{16\} /></button>', button_html, content)

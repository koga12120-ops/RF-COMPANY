    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95 duration-150 max-h-[95vh] flex flex-col">
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              {[25000, 50000, 100000, 250000, 500000].map(val => (
                <button
                  ...
                  className="px-2.5 py-1 bg-slate-100 hover:bg-emerald-50 hover:text-emerald-700 border border-slate-200 rounded-lg text-[11px] font-bold font-mono transition text-slate-700"
                  dir="ltr"
                >
                  +{val.toLocaleString()}
                </button>
          ...
          {/* Real-time Accounting & Distribution Card */}
          {parsedAmount > 0 && (
            <div className="bg-gradient-to-br from-slate-50 to-emerald-50/40 border border-emerald-200/90 rounded-xl p-3.5 space-y-2.5 animate-in fade-in duration-150">
              <div className="flex items-center justify-between text-xs font-bold text-slate-800 border-b border-emerald-200/60 pb-2">
                <span className="flex items-center gap-1.5 text-emerald-800">
                  <PieChart size={15} />
                  <span>دابەشبوونی ژمێریاری (تێچوو و قازانج)</span>
                </span>
                <span className="text-[11px] text-slate-500 font-normal">
                  ڕێژە: {(costRatio * 100).toFixed(0)}% تێچوو / {(profitRatio * 100).toFixed(0)}% قازانج
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="bg-white p-2.5 rounded-lg border border-slate-200/80 shadow-2xs">
                  <div className="flex items-center gap-1 text-[11px] text-slate-500 font-medium">
                    <PackageCheck size={13} className="text-slate-600" />
                    <span>تێچووی کاڵا (سەرمایە)</span>
                  </div>
                  <div className="text-sm font-bold font-mono text-slate-800 mt-1" dir="ltr">
                    {costAmount.toLocaleString()} <span className="text-[10px] font-sans text-slate-500">د.ع</span>
                  </div>
                </div>

                {/* Realized Profit */}
                <div className="bg-emerald-50/70 p-2.5 rounded-lg border border-emerald-200 shadow-2xs">
                  <div className="flex items-center gap-1 text-[11px] text-emerald-700 font-medium">
                    <TrendingUp size={13} className="text-emerald-600" />
                    <span>قازانجی بەدەستهاتوو</span>
                  </div>
                  <div className="text-sm font-black font-mono text-emerald-700 mt-1" dir="ltr">
                    +{profitAmount.toLocaleString()} <span className="text-[10px] font-sans text-emerald-600">د.ع</span>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-emerald-200/60 space-y-1 text-xs">
                ...
                <div className="flex justify-between pt-1 border-t border-slate-200 font-black text-slate-800">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">
                ژمارەی وەسڵ / دەفتەر (ئارەزوومەندانە)
              </label>
              <div className="relative">
                <input
                  type="text"
                  placeholder="ژمارەی دەفتەر وەسڵ..."
                  className="w-full pr-8 pl-3 py-2 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-xs font-mono"
                  value={receiptNo}
                  onChange={(e) => setReceiptNo(e.target.value)}
                />
                <FileText className="absolute right-2.5 top-2.5 text-slate-400" size={14} />
              </div>
            </div>
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="هەر تێبینییەک لەسەر ئەم قەرزدانەوەیە..."
              className="w-full px-3 py-2 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-xs"
              value={notes}

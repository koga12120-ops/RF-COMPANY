  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      const totalValue = cart.reduce((acc, curr) => {
        const unit = (curr as any).unit || 'piece';
        const totalPieces = unit === 'carton' ? curr.quantity * (curr.item.ratio || 1) : (unit === 'packet' ? curr.quantity * (curr.item.packetRatio || 1) : curr.quantity);
        return acc + (curr.item.costPrice * totalPieces);
      }, 0);

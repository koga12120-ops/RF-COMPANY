...
const visits = { 'm1': true, 'm4': true, 'm2': true }; // m2 visited today
...
WEEK_DAYS.forEach(day => {
  ...
  originalMarkets.forEach(mId => {
    if (isPast(day) && !visits[mId]) {
      // move to today
      displaySchedule[currentDayStr].push({ mId, originalDay: day });
    } else {
      displaySchedule[day].push({ mId, originalDay: day });
    }
  });
});
